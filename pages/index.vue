<template>
  <form
    @submit.prevent="newObservation()"
    class="grid gap-9 max-w-[80%] mx-auto"
  >
    <div class="flex flex-col gap-5">
      <SearchBar
        ...
        searchbarUtility="newObservation"
      />
      <input
        type="date"
        name="date"
      ...
      <Transition name="fade" appear>
        <p v-if="badDateMessage" class="text-xl text-center text-red-500">
          {{ badDateMessage }}
        </p>
      </Transition>
    </div>
    ...
    <Transition name="fade" appear>
      <p v-if="errorMessage" class="text-xl text-center text-red-500">
        {{ errorMessage }}
      </p>
    </Transition>
    <Transition name="fade" appear>
...
const newObservation = async () => {
  if (
    ...
    if (!selectedSpecies.value) {
      errorMessage.value = "Veuillez sélectionner une espèce";
    } else if (!date.value) {
      errorMessage.value = "Veuillez sélectionner une date";
    } else if (badDateMessage.value) {
      errorMessage.value = "La date doit être inférieure à la date du jour";
    } else if (!number.value) {
      errorMessage.value = "Veuillez renseigner le nombre d'individus";
    }
