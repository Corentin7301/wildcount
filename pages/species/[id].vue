      ...
      <ul class=" grid gap-11 overflow-y-scroll snap-y scroll-pt-2 calc-max-h no-scroll">
        <li v-for="(observation,index) in observations.data.Observation" :key="index">
          ...
          <Transition name="slide-top" appear>
            <CtaModal v-if="deleteModale" @close-modal="deleteModale = false, justRefreshObservations()"
              message="Veux-tu vraiment supprimer cette observation ?<br/>Il sera impossible de la récupérer !"
              ctaMessage="Supprimer l'observation" :warn="true" job="deletingObservation"
              :observationId="observation.id" />
          </Transition>
          <!--Edit modale-->
          <Transition name="slide-top" appear>
            <CtaModal v-if="editModale" @close-modal="editModale = false"
              message="Veux-tu vraiment supprimer cette observation ?<br/>Il sera impossible de la récupérer !"
              ctaMessage="Supprimer l'observation" job="editObservation" />
          </Transition>
          <!--View Comment modale-->
          <Transition name="slide-top" appear>
            <CtaModal v-if="viewCommentModale" @close-modal="viewCommentModale = false"
              message="Veux-tu vraiment supprimer cette observation ?<br/>Il sera impossible de la récupérer !"
              ctaMessage="Supprimer l'observation" job="deletingObservation" />
          </Transition>
          <div
            ...
            <!-- comment btn -->
            <span @click="viewCommentModale = true"
...
<script setup>
  ...
  const justRefreshObservations = () => refreshNuxtData('speciesObservations')
